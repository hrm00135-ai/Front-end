...
const Leaves = () => {
  ...
  const [typeForm, setTypeForm] = useState({ name: "", code: "", annual_quota: 12, is_paid: true, is_carry_forward: false });
  ...
  const fetchPending = async () => {
    setLoading(true);
    try {
      const res = await apiCall("/leaves?status=pending");
  ...
  const fetchHolidays = async () => {
    try {
      const res = await apiCall("/attendance/holidays");
      const data = await res.json();
  ...
  const handleAddHoliday = async () => {
    try {
      const res = await apiCall("/attendance/holidays", { method: "POST", body: JSON.stringify(holidayForm) });
      const data = await res.json();
      if (data.status === "success") {
          ...
          {showTypeForm && (
            <div className="bg-white p-5 rounded-xl shadow mb-6" style={{ maxWidth: "500px" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                ...
                <FI label="Code (e.g. CL, SL, EL)" value={typeForm.code} onChange={v => setTypeForm({ ...typeForm, code: v.toUpperCase() })} />
                <FI label="Annual Quota (days)" type="number" value={typeForm.annual_quota} onChange={v => setTypeForm({ ...typeForm, annual_quota: parseInt(v) })} />
                ...
                <label style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "14px" }}>
                  <input type="checkbox" checked={typeForm.is_carry_forward} onChange={e => setTypeForm({ ...typeForm, is_carry_forward: e.target.checked })} /> Carry Forward
                </label>
              ...
              <tbody>
                {leaveTypes.length === 0 ? (
                  ...
                  <tr key={lt.id} style={{ borderTop: "1px solid #e2e8f0" }}>
                    ...
                    <td style={td}>{lt.annual_quota}</td>
                    <td style={td}>{lt.is_paid ? "Yes" : "No"}</td>
                    <td style={td}>{lt.is_carry_forward ? "Yes" : "No"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
