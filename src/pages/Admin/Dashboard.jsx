  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        if (empData?.status   === "success") setEmployees(Array.isArray(empData.data) ? empData.data : []);
        if (metalData?.status === "success") setMetals(metalData.data || []);
        if (taskData?.status  === "success") setTasks(Array.isArray(taskData.data) ? taskData.data : []);
    ...
    <Layout topBar={<AdminTopBar />}>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        <div onClick={() => setShowEmpList(!showEmpList)} style={{ cursor: "pointer" }}>
          <StatCard label="Total Employees" value={stats?.total_employees || 0} color="#3b82f6" clickable />
        </div>
        <StatCard label="Checked In Today" value={stats?.present_today || 0} color="#16a34a" />
        <StatCard label="Late Today" value={0} color="#ea580c" />
        {isSuperAdmin ? (
          <div onClick={() => setShowAdminList(!showAdminList)} style={{ cursor: "pointer" }}>
            <StatCard label="Total Admins" value={employees.filter(e => e.role === "admin").length || 0} color="#8b5cf6" clickable />
          </div>
        ) : (
          <StatCard label="On Leave" value={stats?.on_leave_today || 0} color="#8b5cf6" />
        )}
      </div>
      ...
      {/* Stats Row 2 */}
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        <StatCard label="Tasks Pending"    value={stats?.pending_tasks || 0} />
        <StatCard label="In Progress"      value={stats?.pending_tasks || 0} color="#3b82f6" />
        <StatCard label="Completed Today"  value={stats?.completed_tasks_this_month || 0} color="#16a34a" />
        <StatCard label="Overdue"          value={0} color="#dc2626" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="bg-white p-5 rounded-xl shadow">
          ...
          <InfoRow label="Pending Approval" value={stats?.pending_leaves || 0} />
          <InfoRow label="On Leave Today"   value={stats?.on_leave_today || 0} />
        ...
        <div className="bg-white p-5 rounded-xl shadow">
          <h2 className="text-lg font-semibold mb-3">Payroll</h2>
          <InfoRow label="Payslips Generated" value={0} />
          <InfoRow label="Paid"               value={0} />
          <InfoRow label="Pending Payment"      value={0} />
        </div>
      </div>
