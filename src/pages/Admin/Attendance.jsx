...
const Attendance = () => {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      if (data.status === "success") setEmployees((Array.isArray(data.data) ? data.data : []).filter(u => u.role === "employee"));
    } catch {}
  };
  ...
  const fetchAttendance = async () => {
    ...
    try {
      ...
      const to_date = `${year}-${mon}-${String(lastDay).padStart(2, "0")}`;
      const res = await apiCall(`/attendance/user/${selectedEmp}?from_date=${from_date}&to_date=${to_date}`);
      const data = await res.json();
  ...
  const fetchSummary = async () => {
    if (!selectedEmp) return;
    try {
      const res = await apiCall(`/attendance/summary/monthly?user_id=${selectedEmp}&month=${month.split("-")[1]}&year=${month.split("-")[0]}`);
      const data = await res.json();
  ...
  const handleOverride = async () => {
    if (!overrideForm.edit_reason) { setMsg({ text: "Reason is required", type: "error" }); return; }
    try {
      const res = await apiCall(`/attendance/${overrideModal}`, {
        method: "PUT",
                  ...
                  <tbody>
                    {records.map(r => (
                      <tr key={r.id} style={{ borderTop: "1px solid #e2e8f0" }}>
                        ...
                        <td style={td}>
                          <button onClick={() => { setOverrideModal(r.id); setOverrideForm({ check_in_time: r.check_in_time || "", check_out_time: r.check_out_time || "", status: r.status || "present", edit_reason: "" }); }}
      ...
      {overrideModal && (
        <Modal title="Override Attendance" onClose={() => setOverrideModal(null)}>
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            ...
            <FormInput label="Reason *" value={overrideForm.edit_reason} onChange={v => setOverrideForm({ ...overrideForm, edit_reason: v })} />
