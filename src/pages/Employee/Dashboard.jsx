...
const EmployeeDashboard = () => {
  ...
  useEffect(() => {
    const autoCheckIn = async () => {
      try {
        const res = await apiCall("/attendance/checkin", { method: "POST" });
        const data = await res.json();
        if (data.status === "success") {
          setAttendance(data.data);
        } else {
          const todayRes = await apiCall("/attendance/today");
      ...
      try {
        ...
        for (let i = 0; i < results.length; i++) {
          ...
          try {
            ...
            if (i === 1) setTasks(Array.isArray(data.data) ? data.data : []);
            if (i === 2) setAttendance(data.data);
            if (i === 3) setLeaveBalance(Array.isArray(data.data) ? data.data : []);
  ...
  const fetchMyLeaves = async () => {
    try {
      const res = await apiCall("/leaves/my");
      const data = await res.json();
  ...
  const fetchAttHistory = async () => {
    ...
    try {
      ...
      const res = await apiCall(
        `/attendance/my?from_date=${from_date}&to_date=${to_date}`,
      );
      const data = await res.json();
      if (data.status === "success")
  ...
  const fetchPayslips = async () => {
    try {
      const res = await apiCall(`/payroll/payslips/me`);
      const data = await res.json();
      if (data.status === "success") {
        setPayslips(
    ...
    try {
      const res2 = await apiCall(`/payroll/payslips/me`);
  ...
  const fetchSalaryStructure = async () => {
    try {
      const res = await apiCall(`/payroll/salary-structures/me`);
      const data = await res.json();
  ...
  const handleApplyLeave = async () => {
    ...
    try {
      ...
      if (data.status === "success") {
        ...
        fetchMyLeaves();
        // refresh balance
        const balRes = await apiCall("/leaves/balances/me");
        const balData = await balRes.json();
        if (balData.status === "success")
          setLeaveBalance(Array.isArray(balData.data) ? balData.data : []);
      } else {
        setMsg({ text: data.message, type: "error" });
      }
    } catch {
