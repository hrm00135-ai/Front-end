...
const Payments = () => {
  ...
  const fetchSummary = useCallback(async (empId, fd, td) => {
    ...
    try {
      ...
      const res = await apiCall(`/payments/earnings/${empId}?${params}`);
      const data = await res.json();
  ...
  const fetchHistory = useCallback(async (empId, page = 1) => {
    if (!empId) return;
    try {
      const res = await apiCall(`/payments?employee_id=${empId}`);
      const data = await res.json();
      if (data.status === "success") {
        setHistory(Array.isArray(data.data) ? data.data : []);
        setHistoryTotal(Array.isArray(data.data) ? data.data.length : 0);
        setHistoryPage(page);
      }
    } catch {}
  }, []);
  ...
  const fetchEmpTasks = useCallback(async (empId) => {
    if (!empId) return;
    try {
      const res = await apiCall(`/tasks?assigned_to=${empId}`);
      const data = await res.json();
      if (data.status === "success") {
        setEmpTasks(Array.isArray(data.data) ? data.data : []);
      }
    } catch {}
  }, []);
  ...
  const handleRecordPayment = async () => {
    ...
    try {
      ...
      const res = await apiCall("/payments", { method: "POST", body: JSON.stringify(body) });
      const data = await res.json();
      if (data.status === "success") {
  ...
  const handleSaveConfig = async () => {
    if (!selectedEmp) return;
    try {
      const res = await apiCall(`/payments/config`, {
        method: "POST",
        body: JSON.stringify({ ...configForm, user_id: parseInt(selectedEmp), wage_amount: parseFloat(configForm.wage_amount) }),
      });
      const data = await res.json();
      if (data.status === "success") {
  ...
  const handleReverse = async (txId) => {
    ...
    try {
      const res = await apiCall(`/payments/${txId}/reverse`, { method: "POST", body: JSON.stringify({ reason: "Admin reversal" }) });
      const data = await res.json();
      if (data.status === "success") {
